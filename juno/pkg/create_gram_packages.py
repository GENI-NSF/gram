...
class PackageCreator:
    ...
    def run(self):
        ...
        # Generate the three deb files
        template = "python createdpkg.py --node=%s --gcf_root=%s --mon_root=%s --deb_filename=%s/gram_%s.deb --version=%s --gram_root=%s"
        control_command = template % \
            ("control", self.opts.gcf_root, self.opts.mon_root, self.opts.output_directory, \
                 "control", self.opts.version, self.opts.gram_root)
        compute_command = template % \
            ("compute", self.opts.gcf_root, self.opts.mon_root, self.opts.output_directory, \
                 "compute",  self.opts.version, self.opts.gram_root)
        network_command = template % \
            ("network", self.opts.gcf_root, self.opts.mon_root, self.opts.output_directory, \
                 "network",  self.opts.version, self.opts.gram_root)     
        self._execCommand(control_command)
        self._execCommand(compute_command)
        self._execCommand(network_command)
